...
//  Serial.print("Nomor golongan :" + String(golongan));
  ...
  while(client.connected()) {         // Start client.connected()
    if(client.available()){           // Start client.avaliable()
      ...
      const size_t capacity = JSON_OBJECT_SIZE(18) + 410; //cari dulu nilainya pakai Arduino Json 5 Asisten
      DynamicJsonDocument root(capacity);
      // StaticJsonDocument<192> doc;
      DeserializationError error = deserializeJson(root, getData);
//const size_t capacity = JSON_OBJECT_SIZE(18) + 410;


const char* id_rfid = root["id_rfid"]; // "2"
const char* nama_lengkap = root["nama_lengkap"]; // "Benedetto"
const char* alamat = root["alamat"]; // "Jl. Joglo Raya No. 12"
const char* telepon = root["telepon"]; // "081947588234"
const char* foto_ktp = root["foto_ktp"]; // ""
const char* saldo = root["saldo"]; // "5321000"
const char* nomor_rfid = root["nomor_rfid"]; // "1ACE980"
const char* nama_tol = root["nama_tol"]; // "Tegal"
const char* id_pembayaran = root["id_pembayaran"]; // "4929"
const char* id_tol = root["id_tol"]; // "811"
const char* tanggal_transaksi = root["tanggal_transaksi"]; // "2021-12-17 10:24:57"
const char* status = root["status"]; // "Berhasil"
const char* keterangan = root["keterangan"]; // "Transaksi berhasil"
const char* saldo_akhir = root["saldo_akhir"]; // "5321000"
const char* no_transaksi = root["no_transaksi"]; // "INVTOL 2 812 10 102457"
const char* id_golongan = root["id_golongan"]; // "5"
const char* tarif = root["tarif"]; // "11000"
const char* nama_golongan = root["nama_golongan"]; // "golongan_1"
